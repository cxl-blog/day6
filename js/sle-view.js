...
$(document).ready(function () {
    $("#page").on("click","a",function (event) {
        var seledata=getQueryUrl('seledata');
        var page = $(event.target).attr("name");
        window.location='?controller=User&action=sle&seledata='+seledata+"&page="+page;
    });
    ...
    $(".tablebody").on("click",".mange button", function (event) {//修改  删除
        ...
        if ($(clickNode).attr("id") == "mange2") {//删除
            ...
            $.get(url, {}, function () {
                $.post("index.php?controller=User&action=sle&seledata="+getQueryUrl('seledata')+"&page="+getQueryUrl('page'),{delrequest:'del'},function (data) {
                    $(".tablebody").html("");
                    $("#page").html("");
