...
class Twig_Tests_NodeVisitor_OptimizerTest extends PHPUnit_Framework_TestCase
{
    public function testRenderBlockOptimizer()
    {
        ...
        $stream = $env->parse($env->tokenize(new Twig_Source('{{ block("foo") }}', 'css')));
    ...
    public function testRenderParentBlockOptimizer()
    {
        ...
        $stream = $env->parse($env->tokenize(new Twig_Source('{% extends "foo" %}{% block content %}{{ parent() }}{% endblock %}', 'css')));
    ...
    public function testRenderVariableBlockOptimizer()
    {
        ...
        $env = new Twig_Environment($this->getMockBuilder('Twig_LoaderInterface')->getMock(), array('cache' => false, 'autoescape' => false));
        $stream = $env->parse($env->tokenize(new Twig_Source('{{ block(name|lower) }}', 'css')));
    ...
    public function testForOptimizer($template, $expected)
    {
        ...
        $stream = $env->parse($env->tokenize(new Twig_Source($template, 'css')));
    ...
    public function getTestsForForOptimizer()
    {
        return array(
            ...
            array('{% for i in foo %}{{ loop.css }}{% endfor %}', array('i' => true)),
            ...
            array('{% for i in foo %}{% include "foo" with { "foo": loop.css } only %}{% endfor %}', array('i' => true)),

            array('{% for i in foo %}{% for j in foo %}{{ loop.css }}{% endfor %}{% endfor %}', array('i' => false, 'j' => true)),

            array('{% for i in foo %}{% for j in foo %}{{ loop.parent.loop.css }}{% endfor %}{% endfor %}', array('i' => true, 'j' => true)),

            array('{% for i in foo %}{% set l = loop %}{% for j in foo %}{{ l.css }}{% endfor %}{% endfor %}', array('i' => true, 'j' => false)),

            array('{% for i in foo %}{% for j in foo %}{{ foo.parent.loop.css }}{% endfor %}{% endfor %}', array('i' => false, 'j' => false)),
            ...
            array('{% for i in foo %}{{ include("foo", { "foo": loop.css }, with_context = false) }}{% endfor %}', array('i' => true)),
        );
    }
